...
/*
ros2 launch robo_description sin_xz_control.launch.py \
  Kp_x:=100.0 \
  Kp_y:=80.0 \
  Kp_z:=3000.0 \
  Kd_x:=40.0 \
...
public:
  ...
  void control_step(mjData* d)
  {
    ...
    // ===== Stable controller: MuJoCo bias + J^T F + joint posture PD =====
    {
      ...
      Eigen::MatrixXd J_left, J_right, J_left_upper, J_left_forearm;
      Eigen::Vector3d b_left, b_right, b_left_upper, b_left_forearm;
      task_accel_target(left_ee_frame_id_,
                        left_target_pos, left_target_vel, left_target_acc,
                        J_left, b_left);
      task_accel_target(left_upper_frame_id_,
                        l_shape_left_upper_, Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(),
                        J_left_upper, b_left_upper);
      task_accel_target(left_forearm_frame_id_,
                        l_shape_left_forearm_, Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(),
                        J_left_forearm, b_left_forearm);
      task_accel_target(right_ee_frame_id_,
                        right_target_pos, right_target_vel, right_target_acc,
                        J_right, b_right);

      Eigen::MatrixXd J_left_y(1, nv);
      J_left_y.row(0) = J_left.row(1);
      Eigen::VectorXd b_left_y(1);
      b_left_y << b_left.y();

      Eigen::MatrixXd J_left_shape_y(2, nv);
      J_left_shape_y.row(0) = J_left_upper.row(1);
      J_left_shape_y.row(1) = J_left_forearm.row(1);
      Eigen::Vector2d b_left_shape_y;
      b_left_shape_y << b_left_upper.y(), b_left_forearm.y();

      Eigen::MatrixXd J_left_xz(2, nv);
      J_left_xz.row(0) = J_left.row(0);
      J_left_xz.row(1) = J_left.row(2);
      Eigen::Vector2d b_left_xz;
      b_left_xz << b_left.x(), b_left.z();

      // 2) Hierarchical acceleration tasks:
      //    left tool y > left arm shape y > left xz path > right hold > posture.
      add_acc_task(J_left_y, b_left_y, 1e-8);
      add_acc_task(J_left_shape_y, b_left_shape_y, 1e-6);
      add_acc_task(J_left_xz, b_left_xz, 1e-6);
      add_acc_task(J_right, b_right, 1e-5);
      ...
      tau = mujoco_inverse_dynamics(d, qdd_cmd);
...
private:
  ...
  Eigen::VectorXd mujoco_inverse_dynamics(mjData* d, const Eigen::VectorXd& qdd_cmd)
  {
    Eigen::VectorXd tau(pin_model_.nv);
    tau.setZero();

    std::vector<mjtNum> qacc_backup(mj_model_->nv);
    mju_copy(qacc_backup.data(), d->qacc, mj_model_->nv);

    std::fill(d->qacc, d->qacc + mj_model_->nv, 0.0);
    for (size_t i = 0; i < pin_joint_names_.size(); ++i) {
      d->qacc[mj_dof_adr_[i]] = qdd_cmd[pin_v_idx_[i]];
    }

    mj_inverse(mj_model_, d);

    for (size_t i = 0; i < pin_joint_names_.size(); ++i) {
      int pin_v = pin_v_idx_[i];
      if (pin_v < static_cast<int>(pin_v_is_arm_.size()) && pin_v_is_arm_[pin_v]) {
        tau[pin_v] = d->qfrc_inverse[mj_dof_adr_[i]];
      }
    }

    mju_copy(d->qacc, qacc_backup.data(), mj_model_->nv);
    return tau;
  }
...
int main(int argc, char** argv)
{
  ...
  g_ros_node = std::make_shared<rclcpp::Node>("sin_xz_control_node");
  ...
  g_ros_node->declare_parameter("Kp_x", 1200.0);
  g_ros_node->declare_parameter("Kp_y", 6000.0);
  g_ros_node->declare_parameter("Kp_z", 1200.0);
  g_ros_node->declare_parameter("Kd_x", 240.0);
  g_ros_node->declare_parameter("Kd_y", 600.0);
