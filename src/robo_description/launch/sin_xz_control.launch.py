"""
Launch file for sin_xz_control — left-arm figure-8 in the xz plane.

The controller uses hierarchical inverse dynamics: left tool y first, left
upper/forearm y shape second, left x/z figure-8 third, right end-effector hold
fourth, then joint posture in the remaining null-space to suppress shoulder drift.

Topics published (subscribe in PlotJuggler):
  /sin_xz_control_node/left_ee_actual    — actual left EE pose
  /sin_xz_control_node/left_ee_desired   — desired left EE pose (figure-8)
  /sin_xz_control_node/right_ee_actual   — actual right EE pose
  /sin_xz_control_node/right_ee_desired  — desired right EE pose (static L-shape)
  /joint_states                            — all joint positions/velocities

Override parameters at runtime:
  ros2 launch robo_description sin_xz_control.launch.py \
      Kp_x:=1200.0 Kd_x:=240.0 Kp_y:=6000.0 Kd_y:=600.0 Kp_z:=1200.0 Kd_z:=240.0 \
      Kq_posture:=300.0 Dq_posture:=45.0 \
      figure8_frequency:=0.5 figure8_amplitude_x:=0.15 figure8_amplitude_z:=0.10
"""
import os
...
def generate_launch_description():
    ...
    # ---- Node ----
    sin_xz_control_node = Node(
        package="robo_description",
        executable="sin_xz_control",
        name="sin_xz_control_node",
        output="screen",
        parameters=[{
